 ...
 * Idempotent: checks by name, so it's safe to run even if partial data exists.
 * Does NOT require empty tables — skips individual items that already exist.
 */
return new class extends Migration
{
    ...
    {
        // ---------------------------------------------------------
        // 1. INGREDIENTS — insert only if name doesn't exist yet
        // ---------------------------------------------------------
        $ingredients = [
            // MEATS
        ...
        $ingIds = []; // name => id cache
        $driver = DB::getDriverName();
        foreach ($ingredients as $data) {
            $existing = DB::table('ingredients')->where('name', $data['name'])->first();
            if (!$existing) {
                ...
                $ingIds[$data['name']] = $existing->id;
            }
        }

        // ---------------------------------------------------------
        // 2. PRODUCTS — insert only if name doesn't exist yet
        // ---------------------------------------------------------
        $products = [
            // MEALS
            ...
            ['name' => "Tokwa't Baboy",          'category' => 'snacks',     'price' => 40.00],
            ['name' => 'Hotdog and Egg',         'category' => 'snacks',     'price' => 35.00],
            ['name' => 'Fried Tilapia',          'category' => 'snacks',     'price' => 45.00],
            // DRINKS
            ['name' => 'Kapeng Barako (Hot)',    'category' => 'drinks',     'price' => 20.00],
        ...
        foreach ($products as $data) {
            ...
            if (!$existing) {
                ...
                $prodIds[$data['name']] = $existing->id;
            }
        }

        // ---------------------------------------------------------
        // 3. BATCH SIZES & RECIPES
        // Only add if product has no batch size yet
        // ---------------------------------------------------------
        $menu = [
            ['product' => 'Adobong Manok',   'servings' => 5, 'ingredients' => [
            ...
            ['product' => 'Pork Adobo',      'servings' => 5, 'ingredients' => [
            ...
            ['product' => 'Sinigang na Baboy', 'servings' => 6, 'ingredients' => [
            ...
            ['product' => 'Pinakbet',         'servings' => 5, 'ingredients' => [
            ...
            ['product' => 'Tinolang Manok',   'servings' => 5, 'ingredients' => [
            ...
            ['product' => 'Paksiw na Bangus', 'servings' => 4, 'ingredients' => [
            ...
            ['product' => 'Beef Kaldereta',   'servings' => 5, 'ingredients' => [
            ...
            ['product' => 'Pansit Bihon',     'servings' => 8, 'ingredients' => [
            ...
            ['product' => 'Pansit Canton',    'servings' => 8, 'ingredients' => [
            ...
            ['product' => 'Lumpia Shanghai',  'servings' => 10, 'ingredients' => [
            ...
            ['product' => "Tokwa't Baboy",    'servings' => 4, 'ingredients' => [
        ...
        foreach ($menu as $item) {
            ...
            // Only add batch if this product has none yet
            $hasBatch = DB::table('batch_sizes')->where('product_id', $productId)->exists();
            ...
            foreach ($item['ingredients'] as [$ingName, $qty]) {
    ...
    public function down(): void
    {
        $seededNames = [
            ...
            'Pansit Bihon','Pansit Canton','Lumpia Shanghai',"Tokwa't Baboy",
            'Hotdog and Egg','Fried Tilapia','Kapeng Barako (Hot)',
        ...
        $ingNames = ['Pork Belly','Ground Pork','Chicken Thigh','Chicken Liver','Beef Short Ribs',
            'Pork Intestine','Bangus (Milkfish)','Tilapia','Shrimp','Hotdog','Spam / Luncheon Meat',
            ...
            'Canton Noodles','All-Purpose Flour','Egg','Evaporated Milk','Tomato Sauce (canned)',
            'Coconut Milk (canned)','Sardines (canned)','Cooking Oil','Water','Soy Sauce',
            'Fish Sauce (Patis)','Vinegar','Black Pepper (ground)','Bay Leaves','Salt','Sugar',
            'Oyster Sauce','Banana Ketchup','Annatto (Atsuete)','Instant Coffee (3-in-1)',
            'Powdered Juice (Pineapple)','Chocolate Powder (Milo)'];
        DB::table('ingredients')->whereIn('name', $ingNames)->delete();
    }
};
